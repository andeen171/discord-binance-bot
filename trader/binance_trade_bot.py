...
class BinanceTradeBot:
    ...
    def __init__(self, key, secret, crypto, info):
        ...
        self.time_offset = self.client.get_server_time()['serverTime'] - int(time.time() * 1000)
    ...
    def create_order(self):
        ...
        """
        symbol_info = self.client.get_symbol_info(f'{self.crypto}BUSD')
        step_size = symbol_info['quotePrecision']
        precision = int(round(-math.log(float(step_size), 10), 0))
        order = {'time': int(time.time() - self.time_offset)}
        if self.status == "SELL":
            order['type'] = "SELL"
